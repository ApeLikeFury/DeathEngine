...
void main()
{
    ...
    rayinfo ray;
    ray.reflection = reflect(camvector, VertexNormal) + (random(vec2(uv.x, uv.y)) * 0.005);
    for (int i = 0; i < 1; i ++)
    {
    ...
    vec4 tex = texture(iTexture, TextureCoordinate);
    vec3 color = GetDiffuse(VertexNormal,LightPosition,uv,1)/2 + ray.color;
